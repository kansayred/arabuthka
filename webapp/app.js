...
async function performGlobalSearch() {
    ...
    if (!query || query.length < 2) {
        searchResults.innerHTML = '<p class="search-hint">Введите минимум 2 символа</p>';
        return;
    }
    ...
    searchResults.innerHTML = results.map(track => `
        <div class="search-result-item" data-preview="${track.previewUrl || ''}">
            ...
            <div class="search-result-info">
                ...
                <div class="search-result-source">🌍 ${track.source}</div>
            </div>
            <div class="search-result-actions">
                ${track.previewUrl ? `<button class="play-btn" onclick="playPreview('${track.previewUrl}')">▶️</button>` : ''}
            </div>
        </div>
    `).join('');
}

// Воспроизвести превью трека
function playPreview(url) {
    if (!url) return;
    audio.src = url;
    audio.play();
    trackTitle.textContent = 'Превью трека';
    trackArtist.textContent = 'iTunes';
    updatePlayButton();
}
...
// Обновляем обработчик navSearch для показа глобального поиска
if (navSearch) {
    // Удаляем старый обработчик
    navSearch.replaceWith(navSearch.cloneNode(true));
    const newNavSearch = document.getElementById('navSearch');
    newNavSearch.addEventListener('click', () => {
        setActiveNav(newNavSearch);
        showGlobalSearch();
    });
}

// Обновляем navHome для скрытия поиска
if (navHome) {
    const oldNavHome = navHome;
    oldNavHome.addEventListener('click', hideGlobalSearch);
}
if (navLibrary) {
    navLibrary.addEventListener('click', hideGlobalSearch);
}
